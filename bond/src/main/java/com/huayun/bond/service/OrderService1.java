...
public class OrderService1 {
    ...
    public MessageProtocol qryOrder(MessageProtocol msg) throws UnsupportedEncodingException {
        ...
        for (Order order : orders) {
            buffer.writeBytes(Unpooled.copiedBuffer(order.getSubmittingPBUID(),0,6,CharsetUtil.UTF_8));
            buffer.writeBytes(Unpooled.copiedBuffer(order.getSecurityID(),0,8, CharsetUtil.UTF_8));
            buffer.writeBytes(Unpooled.copiedBuffer(order.getSecurityIDSource(),0,4,CharsetUtil.UTF_8));
            buffer.writeShort(order.getOwnerType());
            buffer.writeBytes(Unpooled.copiedBuffer(order.getClearingFirm(),0,2,CharsetUtil.UTF_8));
            buffer.writeLong(order.getTransactTime());
            buffer.writeBytes(Unpooled.copiedBuffer(order.getUserInfo(),0,8,CharsetUtil.UTF_8));
            buffer.writeBytes(Unpooled.copiedBuffer(order.getClOrdID(),0,10,CharsetUtil.UTF_8));
            buffer.writeBytes(Unpooled.copiedBuffer(order.getAccountID(),0,12, CharsetUtil.UTF_8));
            buffer.writeBytes(Unpooled.copiedBuffer(order.getBranchID(),0,4,CharsetUtil.UTF_8));
            buffer.writeBytes(Unpooled.copiedBuffer(order.getOrderRestrictions(),0,4,CharsetUtil.UTF_8));
